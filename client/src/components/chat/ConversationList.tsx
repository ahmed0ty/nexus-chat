...
import { useAuthStore } from "@/stores/authStore";
import { getSocket } from "@/lib/socket";
import { Conversation, ApiResponse, User } from "@/types";
import { cn, formatMessageTime, truncateText, getInitials } from "@/lib/utils";
...
export const ConversationList = ({ onSelectConversation, onNewConversation }: ConversationListProps) => {
  ...
  const getConversationName = (conv: Conversation): string => {
    if (conv.type !== "direct") return conv.name ?? "Unnamed Group";
    const other = conv.participants.find((p) => {
      if (!p.userId || typeof p.userId !== "object") return false;
      return p.userId._id !== user?._id;
    });
    if (!other || typeof other.userId !== "object" || !other.userId) return "Unknown";
    return other.userId.username ?? "Unknown";
  };
  ...
  const getConversationAvatar = (conv: Conversation): string | undefined => {
    ...
    if (!other || typeof other.userId !== "object" || !other.userId) return undefined;
    return other.userId.avatar;
  };
      ...
      <div className="flex-1 overflow-y-auto">
        {conversations.length === 0 ? (
          ...
          conversations.map((conv) => {
            ...
            const isOnline = otherParticipant && typeof otherParticipant.userId === "object"
              ? (otherParticipant.userId as User).isOnline ?? false
              : false;
